...
CREATE TABLE core.users (user_id UUID PRIMARY KEY DEFAULT uuid_generate_v4,
...
CREATE TABLE core.images (image_id UUID PRIMARY KEY DEFAULT uuid_generate_v4,
...
CREATE TABLE core.literature_sources (source_id UUID PRIMARY KEY DEFAULT uuid_generate_v4,
...
CREATE TABLE tagger.attributes (attribute_id UUID PRIMARY KEY DEFAULT uuid_generate_v4,
...
CREATE TABLE tagger.tags (tag_id UUID PRIMARY KEY DEFAULT uuid_generate_v4,
 image_id UUID REFERENCES core.images(image_id),
...
CREATE TABLE evidence.articles (article_id UUID PRIMARY KEY DEFAULT uuid_generate_v4,
...
CREATE TABLE evidence.findings (finding_id UUID PRIMARY KEY DEFAULT uuid_generate_v4,
...
CREATE TABLE evidence.rules (rule_id UUID PRIMARY KEY DEFAULT uuid_generate_v4,
...
CREATE TABLE evidence.rule_evidence (id UUID PRIMARY KEY DEFAULT uuid_generate_v4,
...
CREATE TABLE graphical.model_parameters (param_id UUID PRIMARY KEY DEFAULT uuid_generate_v4,
...
CREATE TABLE graphical.predictions (prediction_id UUID PRIMARY KEY DEFAULT uuid_generate_v4,
 image_id UUID REFERENCES core.images(image_id),
...
CREATE TABLE graphical.mediator_ratings (rating_id UUID PRIMARY KEY DEFAULT uuid_generate_v4,
 image_id UUID REFERENCES core.images(image_id),
...
CREATE TABLE graph.nodes (node_id VARCHAR(100) PRIMARY KEY,
...
CREATE TABLE graph.edges (edge_id VARCHAR(255) PRIMARY KEY,
...
CREATE TABLE graph.edge_evidence (id UUID PRIMARY KEY DEFAULT uuid_generate_v4,
...
function App {
 ...
 return (<div className="ap-shell-gradient">
...
class UnifiedAPIClient {
 ...
 const headers = {
 'Content-Type': 'application/json',...(this.token && { 'Authorization': `Bearer ${this.token}` }),...options.headers,
 };
...
def update_edges_from_findings(findings: List[FindingInput]):
 ...
 EDGES[edge_id] = update_edge_posterior(EDGES[edge_id],
 finding.effect_size,
 finding.weight)
